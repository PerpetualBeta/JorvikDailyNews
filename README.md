...
- **The extractor asks the DOM whether it is ready rather than waiting to be told.** WebKit's "finished loading" callback was only ever a proxy for "the document is parsed", and on macOS 27 the proxy stopped tracking the thing it stands for: measured on a reporter's machine, `document.readyState` read `complete` while `estimatedProgress` sat at 0.10 and `isLoading` stayed true, so the callback never arrived and extraction waited out its whole 10-second timeout on a page that had been ready almost at once. Four articles, four fallbacks, no reader views. A 100 ms poll now reads `document.readyState` and extracts as soon as it reaches `interactive` or `complete`; the callback stays as the fast path where it works, and a one-shot guard stops the two racing. Accepting `interactive` is safe rather than merely convenient here: page scripts are disabled and every subresource is refused, so nothing can add to the document after parsing ends, and the DOM at `interactive` is the final DOM.
...
- Hero images load through a process-wide `ImageCache` that decodes into an `NSCache` and coalesces concurrent requests for a URL onto one in-flight task — so a page turn doesn't re-download, and the lead's prefetch and on-screen view never double-fetch. Pictures are decoded **scaled**, at no more than 2048 pixels on the long edge (`imageMaxPixelSize`), which is what a full-width lead needs on a 2× display: `Paper.maxWidth` less its padding on both sides is 1004 points. `NSImage(data:)` decodes at whatever resolution the source happens to be and holds it, so a 6000-pixel CDN original cost 77.2 MB as a bitmap where the scaled decode costs 9.0 MB, measured on one. The result is then checked against the cap and rescaled if it exceeds it, rather than assuming the decoder honoured the limit: macOS 27 treats the request as a point value and multiplies by the display scale, so asking for 2048 returns 4096 and a picture arrives four times larger than intended. Measured on a reporter's machine before this check existed, all nineteen pictures in one session decoded **larger** than the files they came from, holding 285 MB against a 256 MB limit with ten evictions. The cache is capped in **bytes** as well as in count, at a thirty-second of physical memory — 256 MB on an 8 GB Mac, 1152 MB on a 36 GB one — because `countLimit` bounds how many pictures are held and says nothing at all about how much memory they take.
