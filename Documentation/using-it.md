...
**The reader never leaves you on a pane with nothing in it.** When an article cannot be extracted, the live page is shown instead, and that page is then measured rather than assumed. If it lays out no text and no picture, you get an explanation. If it paints a picture or two and carries not one word of text, you get a different one, because that is a paywall, a check that you are not a robot, or a page that writes its words with its own scripts, and from inside the app those three look identical. It says what is known and offers your browser rather than naming a cause it cannot see.

Media alone used to be enough to call a page drawn. Measured across 51 real live-page loads, ten produced **zero** characters of text and the lowest non-zero result was 102, with nothing in between: a WSJ paywall, The Economist, two Reddit threads, thestar.com, mastodon.social, caffenol.app, AccuWeather with 67 painted media elements, MDPI, and one page of playing cards drawn entirely by script. Not one was worth reading, and each was shown without comment. A picture does not count towards this at all, and two versions of the rule learned that. The first showed any page that painted a picture; the second asked for a picture plus at least one character, on the strength of that gap. A LessWrong comment permalink then drew **one** character beside three pictures, landed in the gap, and showed a blank pane. A gap in 51 samples is not a law. So the only question asked is whether there is something to read, and a page that is one photograph and no words gets the notice too.
