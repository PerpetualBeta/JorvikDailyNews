...
Pictures are never scaled above their own pixel size — a small site logo draws small, sharp and centred rather than blurred to fill the column — and are then held to a height cap: 320 points for the lead, 260 for a card. A picture over its cap is cropped around whatever it actually shows, using Vision's attention saliency, so a phone screenshot crops to the dialogue in the middle rather than the status bar; when Vision finds nothing salient the crop stays top-aligned. Both caps can be retuned on a running copy with the `leadHeroMaxHeight` and `cardImageMaxHeight` defaults keys, and a value of 0 removes the cap. Sub-48-pixel images (tracking pixels, broken CDN placeholders) are rejected so they don't blot the page with empty rectangles. Images that fail to load collapse their slot — the headline rises into the vacated space.
