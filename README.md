...
- **The extractor's web view is put into the app's own window, because WebKit will not load into one that is not.** Apple tightened this in iOS 16 — the forum thread is titled "iOS 16 kills WKWebView instances unattached to a ViewController", and the reporter's case is a headless web view used only to scrape, which is this one exactly — and the same policy has since arrived on macOS. WebKit says so in its own log: "Not eagerly reloading the view because it is not currently visible." On macOS 26 an unattached view still loads. On macOS 27 it does not, and it fails silently: progress parks at 10%, loading stays true, no callback arrives, and the DOM stays at the 39-character skeleton a web view starts with. The view is sized to the window, given an alpha of 0.01 rather than 0 so WebKit still counts it as visible, inserted **below** every other subview so it cannot swallow a click, and removed on every path that ends an extraction.
