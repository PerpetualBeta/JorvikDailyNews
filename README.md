...
**The app is sandboxed, so all of that lives in a container.** From 1.4.8 everything the app stores is under `~/Library/Containers/cc.jorviksoftware.JorvikDailyNews/Data/`, and the app can read nothing outside it except what you hand it through a file picker. Your existing subscriptions, read marks and classifier training are copied across once on the first launch after upgrading, and the log says how many files moved.
